...
{
    public class AsyncMethodOverloadsTests
    {
        private async Task XParams_CallbackAndAssertShouldWork(params int[] ints)
        {
            ...
            var setupMethod = mock
                .Setup.GetType()
                .GetMethod(nameof(SealedOverloadsClass.AsyncReturn), argIntTypes)
            ...
            Task ret = (Task)
                sealedClass
                    .GetType()
                    .GetMethod(nameof(SealedOverloadsClass.AsyncReturn), intTypes)
