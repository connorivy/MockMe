namespace MockMe.Mocks.archive;

public abstract class VoidMemberMock<TSelf, TCallback> : IMemberMockWithCallback<TSelf, TCallback>
{
    List<TCallback>? IMemberMockWithCallback<TSelf, TCallback>.Callbacks { get; set; }
}

public class VoidMemberMock<TArg1>
    : VoidMemberMock<VoidMemberMock<TArg1>, Action<TArg1>>,
        IMemberWithArgsMock<TArg1, VoidMemberMock<TArg1>> { }

public class VoidMemberMock<TArg1, TArg2>
    : VoidMemberMock<VoidMemberMock<TArg1, TArg2>, Action<TArg1, TArg2>>,
        IMemberWithArgsMock<TArg1, TArg2, VoidMemberMock<TArg1, TArg2>> { }

public class VoidMemberMock<TArg1, TArg2, TArg3>
    : VoidMemberMock<VoidMemberMock<TArg1, TArg2, TArg3>, Action<TArg1, TArg2, TArg3>>,
        IMemberWithArgsMock<TArg1, TArg2, TArg3, VoidMemberMock<TArg1, TArg2, TArg3>> { }

public class VoidMemberMock<TArg1, TArg2, TArg3, TArg4>
    : VoidMemberMock<
        VoidMemberMock<TArg1, TArg2, TArg3, TArg4>,
        Action<TArg1, TArg2, TArg3, TArg4>
    >,
        IMemberWithArgsMock<
            TArg1,
            TArg2,
            TArg3,
            TArg4,
            VoidMemberMock<TArg1, TArg2, TArg3, TArg4>
        > { }

public class VoidMemberMock<TArg1, TArg2, TArg3, TArg4, TArg5>
    : VoidMemberMock<
        VoidMemberMock<TArg1, TArg2, TArg3, TArg4, TArg5>,
        Action<TArg1, TArg2, TArg3, TArg4, TArg5>
    >,
        IMemberWithArgsMock<
            TArg1,
            TArg2,
            TArg3,
            TArg4,
            TArg5,
            VoidMemberMock<TArg1, TArg2, TArg3, TArg4, TArg5>
        > { }
